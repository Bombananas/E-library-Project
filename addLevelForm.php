...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['contentSubmit'])) {
    ...
    if ($editId > 0) {
        ...
        $stmt = $conn->prepare("INSERT INTO `tbllevel` (`level_name`, `time`, `description`) VALUES (?, ?, ?)");
        if ($stmt) {
...
<body>
    <div class="container" style="width: 100%;
        ...
        <form class="form" action="addLevelForm.php<?php echo $editId > 0 ? '?edit_id=' . $editId : ($levelIdPre > 0 ? '?level_idpre=' . $levelIdPre : ''); ?>" method="post">
            ...
            <label for="levelTime">Level Time
                <input type="time" id="levelTime" name="levelTime" value="<?php echo htmlspecialchars($levelTime ?: $currentTime); ?>" placeholder="Level Time">
            </label>
